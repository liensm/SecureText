<img width="334" height="359" alt="image" src="https://github.com/user-attachments/assets/6c9a7296-2e05-4695-86ce-4b1e85a152ef" /># SecureText | A Cryptography Browser Extension

A browser extension that encrypts and decrypts text using:
- Caesar cipher
- Rail Fence cipher
- Caesar Backyard Fence cipher

It supports selecting and inputting highlighted text:
- Load highlighted text from a tab.
- Choose between Caesar, Rail Fence, Caesar Backyard Fence for encryption/decryption
- Copy transformed output to clipboard.

## Files
- manifest.json
- extension.html
- extstyling.css
- textretriever.js
- logic.js

## User Manual Section
In order for the user to download this extension in their supported browser, 
the user must do the following in order:

1. Download the zip file located in the Github repository
<img width="394" height="298" alt="image" src="https://github.com/user-attachments/assets/9426bc88-32f8-4da3-9c2d-abd9f1f50794" />

2. Extract the zip file to your device inside a folder
<img width="691" height="562" alt="image" src="https://github.com/user-attachments/assets/1774bd78-588c-4c43-b7e7-d434f409d534" />
<img width="614" height="454" alt="image" src="https://github.com/user-attachments/assets/8533bbb6-8870-4802-a49f-772f50fb18a9" />

3. Open the Extensions page in your browser.
<img width="334" height="359" alt="image" src="https://github.com/user-attachments/assets/c9470a2c-ebe8-4940-91a9-a46ac52daffb" />

4. Enable Developer mode.
<img width="1923" height="611" alt="image" src="https://github.com/user-attachments/assets/c1001c91-281c-4934-89e7-2e91ac30e797" />

5. Choose Load unpacked.
<img width="1926" height="708" alt="image" src="https://github.com/user-attachments/assets/aea9dc59-4449-493b-9100-a2c7d07c87b8" />

6. Select the folder where you extracted the downloaded zip file.
<img width="946" height="451" alt="image" src="https://github.com/user-attachments/assets/bd6724f7-908f-433b-bbf6-3cb913938933" />

7. Now enjoy Secureext!
<img width="1276" height="499" alt="image" src="https://github.com/user-attachments/assets/76b670a7-1923-4360-8b20-f6ae41df85f2" />
<img width="334" height="351" alt="image" src="https://github.com/user-attachments/assets/9333cd94-c5a6-46dc-9625-0d5458e32ecf" />

